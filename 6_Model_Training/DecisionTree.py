...
def plot_confusion(cf_matrix, title):
    ...
    plt.savefig(f"Training_Results/DecisionTree/{title}.png")
    plt.show()
...
def train_DT(X_train, y_train, X_test, y_test, hyper=True, title="Base"):
    if hyper:
        ...
        print(f"DecisionTree {title} GridSearch parameters : {parameter}")
        print(f"DecisionTree {title} GridSearch best score : {best_score}")
        clf = tree.DecisionTreeClassifier(criterion=parameter['criterion'], max_depth=parameter['max_depth'] ,class_weight=parameter['class_weight'])
        
    else:
        clf = tree.DecisionTreeClassifier()
    clf = clf.fit(X_train, y_train)
    os.makedirs('Saved_Model/DecisionTree', exist_ok=True)
    filename = f'Saved_Model/DecisionTree/DecisionTree_{title}_model.sav'
    pickle.dump(clf, open(filename, 'wb'))
    return clf

def report(y_true,y_pred,title):
    print(f"DecisionTree {title} Classification Report")
    print(classification_report(y_true, y_pred))

def roc_plot(fpr, tpr, auc, title):
    plt.plot(fpr,tpr,label="AUC="+str(auc))
    plt.title(f"DecisionTree_{title}_ROC&AUC_Testset")
    plt.ylabel('True Positive Rate')
    plt.xlabel('False Positive Rate')
    plt.legend(loc=4)
    plt.savefig(f"Training_Results/DecisionTree/DecisionTree_{title}_ROC&AUC_Testset.png")
    plt.show()     
...
def roc_all(base, under, over):
    ...
    plt.xlabel('False Positive Rate')
    plt.legend(loc=4)
    plt.savefig(f"Training_Results/DecisionTree/roc_DecisionTree.png")
    plt.show() 
...
def PrecisionRecallAll(base, under, over):
    ...
    plt.title("PrecisionRecallAll DecisionTree True label")
    plt.ylabel('Precision')
    plt.xlabel('Recall')
    plt.legend(loc=3)
    plt.savefig(f"Training_Results/DecisionTree/PrecisionRecallAll DecisionTree True label.png")
    plt.show()
...
def PrecisionRecall(y_test, test_pred_proba, title):
    ...
    plt.title(f"PrecisionRecall {title} DecisionTree")
    plt.ylabel('Precision')
    plt.xlabel('Recall')
    plt.legend(loc=3)
    plt.savefig(f"Training_Results/DecisionTree/PrecisionRecall {title} DecisionTree True label.png")
    plt.show()
...
def main(load_model):
    ...
    os.makedirs('Training_Results/DecisionTree', exist_ok=True)
    ...
    if load_model:
        ...
        DT = pickle.load(open("Saved_Model/DecisionTree/DecisionTree_Base_model.sav", 'rb'))
    else:
        print("Training DecisionTree Base ...")
    ...
    plot_confusion(train_cf_matrix, f"DecisionTree_Base_ConfusionMatrix_Trainset")
    plot_confusion(test_cf_matrix, f"DecisionTree_Base_ConfusionMatrix_Testset")
    ...
    if load_model:
        ...
        DT_under = pickle.load(open("Saved_Model/DecisionTree/DecisionTree_Under_model.sav", 'rb'))
    else:
        print("Training DecisionTree Under ...")
    ...
    plot_confusion(train_cf_matrix_under, f"DecisionTree_Under_ConfusionMatrix_Trainset")
    plot_confusion(test_cf_matrix_under, f"DecisionTree_Under_ConfusionMatrix_Testset")
    ...
    if load_model:
        ...
        DT_over = pickle.load(open("Saved_Model/DecisionTree/DecisionTree_Over_model.sav", 'rb'))
    else:
        print("Training DecisionTree over ...")
    ...
    plot_confusion(train_cf_matrix_over, f"DecisionTree_over_ConfusionMatrix_Trainset")
    plot_confusion(test_cf_matrix_over, f"DecisionTree_over_ConfusionMatrix_Testset")
...
if __name__ == '__main__':
    ...
    print("Training DecisionTree Model Complete !")